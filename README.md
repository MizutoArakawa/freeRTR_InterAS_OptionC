...
範囲: /29	255.255.255.248	8	100.50.81.0～100.50.81.7  
  ...
  links:  
    ...
    # ユーザ向け  
    - endpoints: ["VPC1_1:eth1","PE_A:eth1"] 100.50.80.0/30   VPC1_1:100.50.80.1   PE_A:100.50.80.2   B30  
    - endpoints: ["VPC2_1:eth1","PE_B:eth1"] 100.50.81.0/30   VPC2_1:100.50.81.1   PE_B:100.50.81.2   B30  
    - endpoints: ["VPC1_2:eth1","ePE_A:eth1"] 100.50.80.4/30   VPC1_2:100.50.80.5   ePE_A:100.50.80.6   B30  
    - endpoints: ["VPC2_2:eth1","ePE_B:eth1"] 100.50.81.4/30   VPC2_2:100.50.81.5   ePE_B:100.50.81.6   B30  
...
```
■PE_A  
    B100: host間のパケロス無, AS間ルート変化無し, 検知したログはP1とのOSPFネイバーダウンのみ(約30秒後に対向の装置にてネイバーダウン検知)  
    B101: Aルートのhost間のみパケロス有(約34packets), AS間ルート変化無し, 検知したログはP2とのOSPFネイバーダウンのみ(約36秒後に対向の装置にてネイバーダウン検知)  


■PE_B  
    B110: Bルートのhost間のみパケロス有(約33packets), AS間ルート変化無し, 検知したログはP1とのOSPFネイバーダウンのみ(約40秒後に対向の装置にてネイバーダウン検知)  
    B111: host間のパケロス無, AS間ルート変化無し, 検知したログはP2とのOSPFネイバーダウンのみ(約30秒後に対向の装置にてネイバーダウン検知)  


■P1  
    B10: host間のパケロス無, AS間ルート変化無し, 検知したログはASBR1とのOSPFネイバーダウンのみ  
    B100: Aルートのhost間のみパケロス有(約37packets), AS間ルート変化無し, 検知したログはPE_AとのOSPFネイバーダウンのみ(約37秒後に対向の装置にてネイバーダウン検知)  
    B11: Bルートのhost間のみパケロス有(約40packets), AS間ルート変化無し, 検知したログはASBR2とのOSPFネイバーダウンのみ(約40秒後に対向の装置にてネイバーダウン検知)  
    B110: Bルートのhost間のみパケロス有(約42packets), AS間ルート変化無し, 検知したログはPE_BとのOSPFネイバーダウンのみ(約42秒後に対向の装置にてネイバーダウン検知)  


■P2  
    B101: host間のパケロス無, AS間ルート変化無し, 検知したログはPE_AとのOSPFネイバーダウンのみ(約33秒後に対向の装置にてネイバーダウン検知)  
    B111: host間のパケロス無, AS間ルート変化無し, 検知したログはPE_BとのOSPFネイバーダウンのみ(約36秒後に対向の装置にてネイバーダウン検知)  
    B17: Aルートのhost間のみパケロス有(約35packets), AS間ルート変化無し, 検知したログはASBR1とのOSPFネイバーダウンのみ(約35秒後に対向の装置にてネイバーダウン検知)  
    B18: host間のパケロス無, AS間ルート変化無し, 検知したログはASBR2とのOSPFネイバーダウンのみ(約33秒後に対向の装置にてネイバーダウン検知)  


■ASBR1  
    B1: Aルートのhost間のみパケロス有(約33packets), AルートのみAS間ルート変化有, 検知したログはeASBR1とのOSPFネイバーダウン(約36秒後に対向の装置にてネイバーダウン検知)とeASBR1とのLo1で張ったeBGPネイバーダウン(約2～3分後にダウン検知)  
　　　　Aルート: VPC1_1 ～ ASBR1 ～ ASBR2 ～ eASBR2 ～ eASBR1 ～ VPC1_2  
　　　　→ネイバーアップ後、元のルートに遷移  
    B10: Aルートのhost間のみパケロス有(約33packets), AS間ルート変化無し, 検知したログはP1とのOSPFネイバーダウンのみ(約33秒後に対向の装置にてネイバーダウン検知)  
    B17: host間のパケロス無, AS間ルート変化無し, 検知したログはP2とのOSPFネイバーダウンのみ(約33秒後に対向の装置にてネイバーダウン検知)  
    B50: host間のパケロス無, AS間ルート変化無し, 検知したログはASBR2とのOSPFネイバーダウンのみ(約41秒後に対向の装置にてネイバーダウン検知)  
    B51: host間のパケロス無, AS間ルート変化無し, 検知したログはASBR2とのOSPFネイバーダウンのみ(約40秒後に対向の装置にてネイバーダウン検知)  


■ASBR2  
    B1: Bルートのhost間のみパケロス有(約39packets), BルートのみAS間ルート変化有, 検知したログはeASBR2とのOSPFネイバーダウン(約39秒後に対向の装置にてネイバーダウン検知)とeASBR2とのLo1で張ったeBGPネイバーダウン(約2～3分後にダウン検知)  
　　　　Bルート: VPC2_1 ～ ASBR2 ～ ASBR1 ～ eASBR1 ～ eASBR2 ～ VPC2_2  
　　　　→ネイバーアップ後、元のルートに遷移  
    B11: Bルートのhost間のみパケロス有(約33packets), AS間ルート変化無し, 検知したログはP1とのOSPFネイバーダウンのみ(約33秒後に対向の装置にてネイバーダウン検知)  
    B18: host間のパケロス無, AS間ルート変化無し, 検知したログはP2とのOSPFネイバーダウンのみ(約39秒後に対向の装置にてネイバーダウン検知)  
    B50: host間のパケロス無, AS間ルート変化無し, 検知したログはASBR1とのOSPFネイバーダウンのみ(約33秒後に対向の装置にてネイバーダウン検知)  
    B51: host間のパケロス無, AS間ルート変化無し, 検知したログはASBR1とのOSPFネイバーダウンのみ(約39秒後に対向の装置にてネイバーダウン検知)  


■eASBR1  
    B1: Aルートのhost間のみパケロス有(約37packets), AルートのみAS間ルート変化有, 検知したログはASBR1とのOSPFネイバーダウン(約37秒後に対向の装置にてネイバーダウン検知)とASBR1とのLo1で張ったeBGPネイバーダウン(約2～3分後にダウン検知)  
　　　　Aルート: VPC1_2 ～ eASBR1 ～ eASBR2 ～ ASBR2 ～ ASBR1 ～ VPC1_1  
　　　　→ネイバーアップ後、元のルートに遷移  
    B10: Aルートのhost間のみパケロス有(約39packets), AS間ルート変化無し, 検知したログはePE_AとのOSPFネイバーダウンのみ(約39秒後に対向の装置にてネイバーダウン検知)  
    B17: host間のパケロス無, AS間ルート変化無し, 検知したログはePE_BとのOSPFネイバーダウンのみ(約35秒後に対向の装置にてネイバーダウン検知)  
    B50: host間のパケロス無, AS間ルート変化無し, 検知したログはeASBR2とのOSPFネイバーダウンのみ(約36秒後に対向の装置にてネイバーダウン検知)  
    B51: host間のパケロス無, AS間ルート変化無し, 検知したログはeASBR2とのOSPFネイバーダウンのみ(約33秒後に対向の装置にてネイバーダウン検知)  


■eASBR2  
    B1: Bルートのhost間のみパケロス有(約36packets), BルートのみAS間ルート変化有, 検知したログはASBR2とのOSPFネイバーダウン(約36秒後に対向の装置にてネイバーダウン検知)とASBR2とのLo1で張ったeBGPネイバーダウン(約2～3分後にダウン検知)  
　　　　Bルート: VPC2_2 ～ eASBR2 ～ eASBR1 ～ ASBR1 ～ ASBR2 ～ VPC2_1  
　　　　→ネイバーアップ後、元のルートに遷移  
    B11: host間のパケロス無, AS間ルート変化無し, 検知したログはePE_AとのOSPFネイバーダウンのみ(約36秒後に対向の装置にてネイバーダウン検知)  
    B18: Bルートのhost間のみパケロス有(約36packets), AS間ルート変化無し, 検知したログはePE_BとのOSPFネイバーダウンのみ(約36秒後に対向の装置にてネイバーダウン検知)  
    B50: host間のパケロス無, AS間ルート変化無し, 検知したログはeASBR1とのOSPFネイバーダウンのみ(約36秒後に対向の装置にてネイバーダウン検知)  
    B51: host間のパケロス無, AS間ルート変化無し, 検知したログはeASBR1とのOSPFネイバーダウンのみ(約37秒後に対向の装置にてネイバーダウン検知)  


■ePE_A  
    B10: Aルートのhost間のみパケロス有(約34packets), AS間ルート変化無し, 検知したログはeASBR1とのOSPFネイバーダウンのみ(約34秒後に対向の装置にてネイバーダウン検知)  
    B11: host間のパケロス無, AS間ルート変化無し, 検知したログはeASBR2とのOSPFネイバーダウンのみ(約38秒後に対向の装置にてネイバーダウン検知)  


■ePE_B  
    B17:   host間のパケロス無, AS間ルート変化無し, 検知したログはeASBR1とのOSPFネイバーダウンのみ(約35秒後に対向の装置にてネイバーダウン検知)  
    B18: Bルートのhost間のみパケロス有(約37packets), AS間ルート変化無し, 検知したログはeASBR2とのOSPFネイバーダウンのみ(約37秒後に対向の装置にてネイバーダウン検知)  
```
