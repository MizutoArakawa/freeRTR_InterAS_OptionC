...
### ログイン
`docker exec -it clab-freeRTR_InterAS-OptionC-<freeRouterノード名(例: PE_A)> telnet localhost 2323`  
...
例）  
```bash
docker exec -it clab-freeRTR_InterAS-OptionC-PE_A ip address add 172.31.0.2/24 dev eth0
docker exec -it clab-freeRTR_InterAS-OptionC-PE_A ip link set dev eth0 up
```

`telnet 172.31.0.2 2323`  

※ プレフィックスは、各freeRouterのeth255に設定したIPアドレスにしてください。  
